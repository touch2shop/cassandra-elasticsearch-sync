...
@pytest.mark.slow
class TestCassandraToElasticsearchSync:
    ...
    def test_initial_sync_then_incremental_saves(self,
        ...
        product_fixture_cassandra_store.create(new_product)
        product_fixtures.append(new_product)

        sleep(sync_interval_time)

        for product in product_fixtures:
            product_read_from_elasticsearch = product_fixture_elasticsearch_store.read(product.id)
            assert product_read_from_elasticsearch
            assert product == product_read_from_elasticsearch

    def test_initial_sync_then_incremental_saves_and_deletes(self,
            product_fixtures, product_fixture_cassandra_store, product_fixture_elasticsearch_store, sync_interval_time):
        ...
        for product in product_fixtures:
            product_fixture_cassandra_store.delete(product)
        ...
        assert product_fixture_elasticsearch_store.read(new_product.id)
