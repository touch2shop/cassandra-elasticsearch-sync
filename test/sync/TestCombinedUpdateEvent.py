...
class TestCombinedUpdateEvent:
    ...
    def test_should_get_timestamp_from_most_recent_update(self, identifier):
        combined = CombinedUpdateEvent(identifier)
        combined.add_update_event(UpdateEvent(identifier, timestamp=1, field_names={"a"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=5, is_delete=True))
        combined.add_update_event(UpdateEvent(identifier, timestamp=6, field_names={"g"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=3, field_names={"b", "c", "d"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=2, field_names={"b", "c"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=4, field_names={"e", "f"}))

        assert_that(combined.timestamp, equal_to(6))

    def test_is_delete_if_last_update_is_delete(self, identifier):
        combined = CombinedUpdateEvent(identifier)
        combined.add_update_event(UpdateEvent(identifier, timestamp=1, field_names={"a"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=5, is_delete=True))
        combined.add_update_event(UpdateEvent(identifier, timestamp=5, field_names={"a"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=3, field_names={"b", "c", "d"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=2, field_names={"b", "c"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=4, field_names={"e", "f"}))
    ...
    def test_is_not_delete_if_last_update_is_save(self, identifier):
        ...
        combined.add_update_event(UpdateEvent(identifier, timestamp=5, is_delete=True))
        combined.add_update_event(UpdateEvent(identifier, timestamp=5, field_names={"a"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=6, field_names={"a"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=3, field_names={"b", "c", "d"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=2, field_names={"b", "c"}))
        combined.add_update_event(UpdateEvent(identifier, timestamp=4, field_names={"e", "f"}))
